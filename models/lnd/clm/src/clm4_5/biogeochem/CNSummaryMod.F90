...
! !USES:
   ...
   use clm_varpar  , only: nlevdecomp,ndecomp_pools,ndecomp_cascade_transitions
#endif
   use pftvarcon, only: exudate_frac
...
! local pointers to implicit in scalars
   ...
   real(r8), pointer :: cpool_to_turnover(:)          ! turnover of short term C (gC/m2/s)
...
! fire variables changed by F. Li and S. Levis
   ...
   real(r8), pointer :: excess_cflux_to_cpool(:)  !downreg flux to short term storage C 
...
real(r8), pointer :: micbioc_col(:) 
real(r8), pointer :: doc_col(:) 
real(r8), pointer :: dochr_vr(:,:) 
real(r8), pointer :: dochr_col(:) 
real(r8), pointer :: ace_col(:)  
real(r8), pointer :: ace_prod_col(:)  
real(r8), pointer :: acebios_col(:)  
real(r8), pointer :: co2bio_col(:) 
real(r8), pointer :: aerch4bio_col(:) 
real(r8), pointer :: anaerch4bio_col(:) 
...
real(r8), pointer :: ccon_ch4s_col(:)				! column-level concentration of CH4 gC/m2
real(r8), pointer :: ccon_co2s_col(:)				! column-level concentration of CO2 gC/m2
real(r8), pointer :: mic_fco2_col(:)  
real(r8), pointer :: mic_fch4_col(:)  
#endif  
...
!-----------------------------------------------------------------------
   ...
   ! assign local pointers
    ...
    cpool_to_turnover              => pcisof%cpool_to_turnover
...
! fire variables changed by F. Li and S. Levis
...
#if(defined MICROBE)
    m_frootc_to_doc             => pcisof%m_frootc_to_doc
#endif
    ...
    excess_cflux_to_cpool          => pcisof%excess_cflux_to_cpool
    rr                             => pcisof%rr
   ...
   mic_fch4_col           			=> ccf%mic_fch4_col  
   mic_fco2_col           			=> ccf%mic_fco2_col  
#endif 
   ...
   do fp = 1,num_soilp
      ...
      gpp(p) = &
         ...
         psnshade_to_cpool(p)
#ifdef CPOOLMOD
      gpp(p) = gpp(p) + excess_cflux_to_cpool(p)
#endif
...
#ifdef CPOOLMOD
       ar(p) = ar(p) + (1.0_r8 - exudate_frac(ivt(p))) * cpool_to_turnover(p)
#endif
      ...
      npp(p) = gpp(p) - ar(p)
#ifdef CPOOLMOD
      npp(p) = npp(p) - exudate_frac(ivt(p)) * cpool_to_turnover(p)
#endif
...
!~ #if(defined MICROBE)
	!~ litfall(p) = litfall(p) + m_frootc_to_doc(p)
!~ #endif
...
#ifdef MICROBE
      dochr_col(c) = 0._r8
      micbiohr_col(c) = 0._r8
      micbioc_col(c) = 0._r8
      doc_col(c) = 0._r8
      ace_col(c) = 0._r8      
      ace_prod_col(c) = 0._r8      
      ccon_ch4s_col(c) = 0._r8
      ccon_co2s_col(c) = 0._r8
      mic_fch4_col(c) = 0._r8
      mic_fco2_col(c) = 0._r8
#endif
...
#ifdef MICROBE
!	    micbio_hr(c,k) = micbio_hr(c,k) + micbio_hr_vr(c,j,k)*dzsoi_decomp(j) 
#endif
         end do
      end do
   end do
   ...
   do k = 1, ndecomp_cascade_transitions
      if ( is_microbe(cascade_donor_pool(k)) ) then
         do fc = 1,num_soilc
            c = filter_soilc(fc)
		micbiohr_col(c) = micbiohr_col(c) + decomp_cascade_hr(c,k)
		!write(iulog,*) "micbiohr_col: ", micbiohr_col(c)
          end do
	end if
    end do
#endif
...
#ifdef CLOSEDCHAMBER   !Sweden closed chamber experiment (CO2 is drawn down by NEP)
       if ((year == 2007 .or. year == 2008)) then 
         if ((mon == 6 .and. day == 14) .or. (mon == 8 .and. day == 14)) then 
	   if (sec .le. 86700 / 2) then 
	     co2_ppmv = 550._r8
	     print*, 'CO2_OUT', year, mon, day, sec, nep(C)*3600, co2_ppmv	
           else if (sec .ge. 87600 / 2) then  
	     if (year == 2007) co2_ppmv = max(co2_ppmv - nep(c) / (4.0_r8) / 12._r8 * 300._r8 / 40.8 * 1.0e6_r8, 0._r8)
             if (year == 2008) co2_ppmv = max(co2_ppmv - nep(c) / (5.0_r8) / 12._r8 * 300._r8 / 40.8 * 1.0e6_r8, 0._r8)
	     print*, 'CO2_OUT', year, mon, day, sec, nep(C)*3600*44/12._r8, co2_ppmv
           end if
         else
           co2_ppmv = 375._r8
         end if
   ...
   do j = 1, nlevdecomp
      ...
      do j = 1, nlevdecomp
          do fc = 1,num_soilc
             c = filter_soilc(fc)
!          micbioc_col(c) = micbioc_col(c) + cmicbiocs(c,j) * dzsoi_decomp(j)
          doc_col(c) = doc_col(c) + cdocs(c,j) * dzsoi_decomp(j)
	  ace_col(c) = ace_col(c) + caces(c,j) * dzsoi_decomp(j)
!	  !ace_prod_col(c) = ace_prod_col(c) + caces_prod(c,j) * dzsoi_decomp(j)
	  ccon_ch4s_col(c) = ccon_ch4s_col(c) + ccon_ch4s(c,j) * dzsoi_decomp(j)
	  ccon_co2s_col(c) = ccon_co2s_col(c) + ccon_co2s(c,j) * dzsoi_decomp(j)
	   end do
      end do
#endif
...
! fire related variables changed by F. Li and S. Levis
   ...
   real(r8), pointer :: decomp_npools_transport_tendency(:,:,:)    ! N tendency due to vertical transport in decomposing N pools (gN/m^3/s)
	...
	real(r8), pointer :: cmicbions(:,:) 
	real(r8), pointer :: cdons(:,:)   
	real(r8), pointer :: micbion_col(:) 
	real(r8), pointer :: don_col(:) 
#endif  
...
!-----------------------------------------------------------------------
...
#if(defined MICROBE)
      totpftn(p) = totpftn(p) + m_frootn_to_don(p)
#endif
   ...
   do fc = 1,num_soilc
      ...
      do j = 1, nlevdecomp
         do fc = 1,num_soilc
            c = filter_soilc(fc)
            micbion_col(c) = micbion_col(c) + cmicbions(c,j) * dzsoi_decomp(j)
            don_col(c) = don_col(c) + cdons(c,j) * dzsoi_decomp(j)     
         end do
      end do
#endif
