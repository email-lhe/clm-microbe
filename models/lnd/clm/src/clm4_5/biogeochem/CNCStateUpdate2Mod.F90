...
#if(defined MICROBE)
    litter_c_to_doc  			=> ccisof%litter_c_to_doc
...
#if(defined MICROBE)
!write(*,*) "here 3"
	if(litter_c_to_doc(c,j)>litter_n_to_don(c,j)*cn_dom) then
	decomp_cpools_vr(c,j,i_met_lit) = decomp_cpools_vr(c,j,i_met_lit) + litter_c_to_doc(c,j) - litter_n_to_don(c,j)*cn_dom
	end if
	  decomp_cpools_vr(c,j,i_dom) = decomp_cpools_vr(c,j,i_dom) + litter_c_to_doc(c,j) * dt
!write(*,*) "here 4"
#endif
